...
int dm510_init_module(void)
{
	...
	//allocate
	devices = kmalloc(DEVICE_COUNT * sizeof(struct dev_driver), GFP_KERNEL);
	if (!devices)
	{
		result = -ENOMEM;
...
static int dm510_open(struct inode *inode, struct file *filp)
{
	...
	if (filp->f_mode & FMODE_READ)
	{
		...
		while(dev->readbuf->nreaders >= read_limit){
			...
			if(wait_event_interruptible(dev->readbuf->inq,(dev->readbuf->nreaders < read_limit))){
				return -ERESTARTSYS;
			}
			if (mutex_lock_interruptible(&dev->readbuf->mutex))
			{
